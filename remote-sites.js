        ...
        modal.innerHTML = `
            ...
            <div class="rs-modal__body">
                ...
                <div class="rs-section">
                    ...
                    <label class=\"rs-checkall\"><input type=\"checkbox\" id=\"rsSelectAllDomains\"> Выделить все</label>
                ...
                <div class="rs-section" id="rsContentSection" style="display:none">
                    ...
                    <div class="rs-content-panel active" data-panel="files">
                        <div class=\"rs-row\"><label class=\"rs-checkall\"><input type=\"checkbox\" id=\"rsSelectAllFiles\"> Выделить все файлы</label></div>
                    ...
                    <div class="rs-content-panel" data-panel="links">
                        <div class=\"rs-row\"><label class=\"rs-checkall\"><input type=\"checkbox\" id=\"rsSelectAllLinks\"> Выделить все ссылки</label></div>
...
const rsSelAllDomains = modal.querySelector('#rsSelectAllDomains');
if (rsSelAllDomains) {
    rsSelAllDomains.addEventListener('change', (e) => {
        modal.querySelectorAll('.rs-domain-checkbox').forEach(cb => cb.checked = e.target.checked);
        updateSelectedDomains();
    });
}
const rsSelAllFiles = modal.querySelector('#rsSelectAllFiles');
if (rsSelAllFiles) {
    rsSelAllFiles.addEventListener('change', (e) => {
        const panel = modal.querySelector('.rs-content-panel[data-panel="files"]');
        if (panel) panel.querySelectorAll('.rs-item-checkbox').forEach(cb => cb.checked = e.target.checked);
        updateSelectedItems();
    });
}
const rsSelAllLinks = modal.querySelector('#rsSelectAllLinks');
if (rsSelAllLinks) {
    rsSelAllLinks.addEventListener('change', (e) => {
        const panel = modal.querySelector('.rs-content-panel[data-panel="links"]');
        if (panel) panel.querySelectorAll('.rs-item-checkbox').forEach(cb => cb.checked = e.target.checked);
        updateSelectedItems();
    });
}
    ...
    function switchTab(tab) {
        ...
        // NEW: очистка выбора при смене вкладки
        selectedItems = [];
    ...
    function updateSelectedItems() {
        selectedItems = [];
        document.querySelectorAll(`.rs-content-panel[data-panel="${currentTab}"] .rs-item-checkbox:checked`).forEach(cb => {
...
// NEW: синхронизируем состояние чекбоксов “Выделить всё”
function updateSelectAllStates() {
    // Домены
    const allDomains = document.querySelectorAll('.rs-domain-checkbox');
    const checkedDomains = document.querySelectorAll('.rs-domain-checkbox:checked');
    const masterDomains = document.getElementById('rsSelectAllDomains');
    if (masterDomains) {
        masterDomains.indeterminate = checkedDomains.length > 0 && checkedDomains.length < allDomains.length;
        masterDomains.checked = allDomains.length > 0 && checkedDomains.length === allDomains.length;
    }

    // Элементы в активной вкладке
    const panel = document.querySelector(`.rs-content-panel[data-panel="${currentTab}"]`);
    const allItems = panel ? panel.querySelectorAll('.rs-item-checkbox') : [];
    const checkedItems = panel ? panel.querySelectorAll('.rs-item-checkbox:checked') : [];
    const masterItems = document.getElementById(currentTab === 'files' ? 'rsSelectAllFiles' : 'rsSelectAllLinks');
    if (masterItems) {
        masterItems.indeterminate = checkedItems.length > 0 && checkedItems.length < allItems.length;
        masterItems.checked = allItems.length > 0 && checkedItems.length === allItems.length;
    }
}
